...
class SpanBarProcessor
  ...
  def add(t, p, v = 0, position = false)
    result = false
    ...
    if @checkTrend
      trendStatus = self.check_trend_status(t, p, position)
  ...
  def check_trend_status(t, p, position) 
    result = []
    ...
    if @recentBar.nil? or @recentBar != @spanBars.last
      ...
      if peak[3] == :UP
        ...
        if @bearish
          if @bearish[:resistance].nil? or @bearish[:support].nil?          # initialize if unset
    ...
    if @bullish
      ...
      if @bullish and @bullish[:support] and @bullish[:resistance]
        if    [:stopped, :trailing].include? @bullish[:status] and 
          ...
          # ignore 
        elsif    (position ? (p < @bullish[:support][1] - @grace * @ts) : (p < @bullish[:support][1]))
